...
ceph-authtool "$KEYRING" --gen-key -n client.admin --cap mon 'allow *' --cap osd 'allow *' --cap mds 'allow *'
...
for host in "${hosts[@]}"; do
    ...
    if ssh "root"@"$host" "[ ! -e /var/lib/ceph/mon/ceph-$host/store.db.original ]"; then
        ...
        rsync -avz $ms/store.db/ root@$host:/var/lib/ceph/mon/ceph-$host/store.db/ --delete
